...
const FoodScreen = ({ navigation, route }) => {
    ...
    return (
        <View style={styles.container}>
            ...
            <Image
                style={styles.image}
                source={MenuItemsList?.meal_img}
            />
            <ScrollView>
                <Separator height={Display.setWidth(100)} />
                <View style={styles.mainContainer}>
                    <View style={styles.titleHeaderContainer}>
                        <Text style={styles.titleText}>{MenuItemsList?.meal_name}</Text>
                        <Text style={styles.priceText}>${MenuItemsList?.price}</Text>
                    </View>
                    ...
                    <View style={styles.detailsContainer}>
                        {MenuItemsList?.description ? (
                            <>
                                ...
                                <Text style={styles.detailContent}>{MenuItemsList?.description}</Text>
                            </>
                        ) : null}
                        {MenuItemsList?.ingredients ? (
                            <>
                                <Text style={styles.detailHeader}>Ingredients</Text>
                                <Text style={styles.detailContent}>{MenuItemsList?.ingredients}</Text>
                            </>
                        ) : null}
                    </View>
                </View>
            ...
            <View style={styles.buttonsContainer}>
                <View style={styles.itemAddContainer}>
                    <AntDesign
                        ...
                        color={Colors.DEFAULT_YELLOW}
                        size={18}
                        onPress={() => dispatch(removFromCart(MenuItemsList))}
                    />
                    <Text style={styles.itemCountText}>{handleQuantity()}</Text>
                    <AntDesign
                        name="plus"
                        color={Colors.DEFAULT_YELLOW}
                        size={18}
                        onPress={() => dispatch(addToCart(MenuItemsList))}
                    />
                </View>
                <TouchableOpacity
